...
FeishuClaudeCodeBridge 不能替你自动创建飞书应用。你需要先在飞书开放平台创建一个企业自建应用，开启机器人能力，然后把 `App ID` 和 `App Secret` 提供给安装脚本。
...
安装 FeishuClaudeCodeBridge 时只需要你手动提供这两个值。
...
FeishuClaudeCodeBridge 使用飞书开放平台的长连接 WebSocket 接收消息事件和卡片回调，不需要公网回调地址。

在应用后台里：

1. 打开「事件与回调」。
2. 在「回调配置」或「订阅方式」里选择使用长连接接收事件和回调。
3. 订阅事件：
...
如果点击卡片按钮时客户端提示 `code: 200340`，说明飞书没有把卡片回调路由到 Bridge：要么没有配置回调订阅方式，要么没有订阅 `card.action.trigger`，要么改完配置后还没有发布新版应用。

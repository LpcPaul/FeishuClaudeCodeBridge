#!/usr/bin/env python3
"""Feishu to Claude Code bridge.

This bridge intentionally stays small: it connects Feishu messages to Claude Code,
returns Claude Code's own final text, and manages lightweight topic boundaries for a
...
class BridgeConfig:
    ...
    bot_aliases: tuple[str, ...] = ("Claude Code", "claude", "机器人")
    ...
    def from_env(cls) -> "BridgeConfig":
        return cls(
            ...
            workdir=Path(os.getenv("FEISHU_CLAUDE_CODE_WORKDIR", str(DEFAULT_WORKDIR))).expanduser(),
            runtime_dir=Path(os.getenv("FEISHU_CLAUDE_CODE_RUNTIME_DIR", str(DEFAULT_RUNTIME_DIR))).expanduser(),
            ...
            claude_permission_mode=os.getenv("CLAUDE_PERMISSION_MODE", "auto").strip(),
            claude_effort=os.getenv("CLAUDE_EFFORT", "").strip(),
            claude_max_budget_usd=os.getenv("CLAUDE_MAX_BUDGET_USD", "").strip(),
            claude_allowed_tools=tuple(_csv_env("CLAUDE_ALLOWED_TOOLS")),
            claude_disallowed_tools=tuple(_csv_env("CLAUDE_DISALLOWED_TOOLS")),
            claude_extra_args=tuple(shlex.split(os.getenv("CLAUDE_EXTRA_ARGS", ""))),
            bot_aliases=tuple(_csv_env("FEISHU_BOT_ALIASES") or ["Claude Code", "claude", "机器人"]),
...
class StateStore:
    ...
    def get_session_id(self, session_key: str) -> str | None:
        with self.connect() as con:
            ...
            return str(row["claude_session_id"]) if row and row["claude_session_id"] else None
    ...
    def upsert_session(self, session_key: str, source: str, title: str = "") -> None:
        ...
        with self.connect() as con:
            con.execute(
                """
                insert into sessions(session_key, claude_session_id, title, source, created_at, updated_at)
                values(?, null, ?, ?, ?, ?)
    ...
    def set_session_id(self, session_key: str, session_id: str) -> None:
        with self.connect() as con:
            con.execute(
                ...
                (session_id, now_iso(), session_key),
            )

    def reset_session(self, session_key: str) -> None:
        with self.connect() as con:
            con.execute(
                "update sessions set claude_session_id = null, updated_at = ? where session_key = ?",
                (now_iso(), session_key),
            )
...
class ClaudeCodeRunner:
    ...
    def run(self, prompt: str, session_id: str | None) -> tuple[str | None, str]:
        ...
        parsed = parse_claude_json_result(completed.stdout)
        new_session_id = session_id or parse_claude_session_id(completed.stdout)
        reply = parse_claude_reply(completed.stdout) or completed.stderr.strip()
        is_error = bool(parsed and parsed.get("is_error"))
        if completed.returncode != 0 or is_error:
            detail = reply or f"退出码 {completed.returncode}。"
            reply = f"Claude Code 执行异常终止，退出码 {completed.returncode}。\n\n{detail}".strip()
        return new_session_id, reply.strip()

    def _build_command(self, session_id: str | None) -> list[str]:
        command = [*self._claude_command(), "-p", "--output-format", "json"]
        if session_id:
        ...
        if self.config.claude_effort:
            command.extend(["--effort", self.config.claude_effort])
        if self.config.claude_max_budget_usd:
            command.extend(["--max-budget-usd", self.config.claude_max_budget_usd])
        if self.config.claude_allowed_tools:
            command.append("--allowedTools")
            command.extend(self.config.claude_allowed_tools)
        if self.config.claude_disallowed_tools:
            command.append("--disallowedTools")
            command.extend(self.config.claude_disallowed_tools)
        command.extend(self.config.claude_extra_args)
        return command

    def _claude_command(self) -> list[str]:
        if self.config.node_bin:
            claude_path = Path(self.config.claude_bin).expanduser()
            resolved_path = claude_path.resolve() if claude_path.exists() else claude_path
            if str(claude_path).endswith(".js") or str(resolved_path).endswith(".js"):
                return [self.config.node_bin, str(resolved_path)]
        return [self.config.claude_bin]
...
def build_prompt(envelope: MessageEnvelope, route: RouteDecision) -> str:
    return (
        "这是一条从飞书转发到 Claude Code 的用户消息。\n"
        "按当前工作区规则处理；不要把飞书入口绑定到任何单一技能或工具。\n"
        "如果用户明确要求调研、读取链接、编辑文件或运行工具，你可以按 Claude Code 正常能力选择合适方法。\n\n"
...
def handle_command(store: StateStore, route: RouteDecision, text: str) -> str | None:
    ...
    if normalized in {"/new", "新会话", "重新开始"}:
        ...
        return "已重新开始这段 Claude Code 对话。"
    if normalized in {"/clear", "清空上下文"}:
        store.upsert_session(route.session_key, route.reason)
        store.reset_session(route.session_key)
        return "已清空这段飞书容器绑定的 Claude Code 对话。"
    if normalized in {"/status", "当前会话"}:
        session_id = store.get_session_id(route.session_key)
        return f"当前飞书容器：{route.session_key}\nClaude Code 会话：{session_id or '尚未创建'}"
    return None
...
def normalize_mentions(text: str) -> str:
    ...
    cleaned = re.sub(r"^@(Claude Code|Claude|claude|机器人)\s+", "", cleaned)
    return re.sub(r"@\S+\s+", "", cleaned).strip()


def parse_claude_json_result(stdout: str) -> dict[str, Any] | None:
    text = stdout.strip()
    if not text:
        return None
    try:
...
def parse_claude_session_id(stdout: str) -> str | None:
    payload = parse_claude_json_result(stdout)
    if not payload:
        return None
...
def parse_claude_reply(stdout: str) -> str | None:
    payload = parse_claude_json_result(stdout)
    if not payload:
...
class FeishuClaudeCodeBridge:
    def __init__(self, config: BridgeConfig, api_client: Any, reply_request: Any, reply_body: Any) -> None:
        ...
        self.runner = ClaudeCodeRunner(config)
    ...
    def handle_message(self, data: Any) -> None:
        ...
        session_id = self.store.get_session_id(route.session_key)
        prompt = build_prompt(envelope, route)
        self.store.begin_task(route.session_key)
        worker = threading.Thread(
            target=self._run_claude_and_reply,
            args=(envelope, route, session_id, prompt),
            daemon=True,
        )
        try:
            worker.start()
    ...
    def _run_claude_and_reply(
        ...
        try:
            new_session_id, reply = self.runner.run(prompt, session_id)
            if new_session_id and new_session_id != session_id:
                self.store.set_session_id(route.session_key, new_session_id)
            reply = truncate_reply(reply or "Claude Code 没有返回内容。", self.config.reply_max_chars)
        except Exception as exc:
            reply = f"Claude Code 执行失败：{exc}"
        finally:
            progress_stop.set()
            if progress_thread:
...
def run_listener(config: BridgeConfig) -> None:
    ...
    bridge = FeishuClaudeCodeBridge(config, api_client, ReplyMessageRequest, ReplyMessageRequestBody)
    ...
    print(f"Feishu Claude Code bridge is running. DB: {config.db_path}")
    ws_client.start()
